...
const navItems = [
	...
	{
		label: "Github",
		href: "https://github.com/grim-shaw",
			...
			{navItems.map(item => (
				<Link key={item.label} href={item.href}>
					<div className="m-0 mx-[1em] my-auto hidden h-[2.5em] cursor-pointer flex-row justify-center rounded-[10px] border-[1px] border-[#413f3f] px-[1em] align-middle text-[#bbb] hover:border-[#c0bbbb] min-[520px]:flex">
						...
						/>
						<p className="m-0 mx-auto my-auto ml-1">{item.label}</p>
					</div>
				</Link>
			))}
			<div
			...
			>
				...
				>
					{navItems.map((item, index) => (
						<Link key={item.label} href={item.href}>
							<div
								className={`m-auto w-[100%] px-[2em] py-[0.75em] hover:bg-[#18181B] ${
									index === 0 ? "rounded-t-[0.5em]" : ""
								} ${
									index === navItems.length - 1
										? "rounded-b-[0.5em]"
										: "border-b-[1px] border-[#413f3f]"
								}`}
							>
								{item.label}
							</div>
